...
export const DocumentAuditLedger = () => {
  ...
  if (loading) {
    return <div className="p-8 animate-pulse text-emerald-400">Loading Document Repository...</div>;
  }
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-emerald-400">
            ...
            <span>Digital Repository & E-Signed Verification Vault</span>
          </div>
          <h1 className="text-2xl font-extrabold font-heading text-white mt-1">Official Gazette & Document Repository</h1>
          <p className="text-xs text-slate-400">Secure PDF document upload repository with version control and document audit history.</p>
        </div>
        ...
        <div className="flex items-center gap-3 shrink-0 flex-wrap">
          ...
          <div className="flex items-center gap-1 bg-slate-950 p-1 rounded-xl border border-slate-800">
            ...
            >
              ...
              <span>Document Repository & Versions</span>
            </button>
            <button
            ...
            >
              ...
              <span>Document Audit History ({auditLogs.length})</span>
            </button>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleUploadSubmit} className="bg-slate-900/95 border border-slate-800 p-6 rounded-3xl space-y-5 shadow-2xl animate-in fade-in">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <div className="flex items-center gap-2 text-emerald-400 font-bold text-sm">
              ...
              <span>Upload Official Statutory PDF File</span>
            </div>
            <span className="text-xs text-slate-400">PDF Files Only (Max 25MB)</span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <div>
              <label className="text-slate-300 font-semibold mb-1 block">Target Infrastructure Project:</label>
              <select
                value={targetProject}
            ...
            <div>
              <label className="text-slate-300 font-semibold mb-1 block">Document Type / Category:</label>
              <select
                value={docType}
                onChange={(e) => setDocType(e.target.value)}
                className="w-full p-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-emerald-500"
              >
                <option value="Initial Proposal Dossier">Initial Proposal Dossier</option>
                <option value="Social Impact Study Report">Social Impact Study Report</option>
                <option value="Preliminary Gazette Notification">Preliminary Gazette Notification</option>
                <option value="Final Acquisition Declaration Gazette">Final Acquisition Declaration Gazette</option>
                <option value="Land Valuation & Award Dossier">Land Valuation & Award Dossier</option>
                <option value="Rehabilitation Housing Master Plan">Rehabilitation Housing Master Plan</option>
              </select>
            </div>

            <div>
              <label className="text-slate-300 font-semibold mb-1 block">Document Version Tag:</label>
              <select
                value={docVersion}
              ...
              >
                <option value="v1.0">v1.0 (Initial Official Release)</option>
                <option value="v1.1">v1.1 (Minor Correction / Amendment)</option>
                <option value="v2.0">v2.0 (Revised Gazette / Re-issued Award)</option>
              </select>
            </div>

            <div className="md:col-span-3">
              <label className="text-slate-300 font-semibold mb-1 block">Document Title / Notification Reference:</label>
              <input
                type="text"
                required
                value={docTitle}
                onChange={(e) => setDocTitle(e.target.value)}
                placeholder="e.g. Gazette Notification No. S.O. 1928(E) dated 15-Feb-2026"
                className="w-full p-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-emerald-500"
              />
            </div>
            ...
            <div className="md:col-span-3">
              <label className="text-slate-300 font-semibold mb-1 block">Select PDF Document File from Device:</label>
                ...
                <div className="space-y-2 pointer-events-none">
                  ...
                  {selectedFile ? (
                    <div className="space-y-1">
                      ...
                      <span className="text-[10px] text-slate-400">{(selectedFile.size / 1024).toFixed(1)} KB • PDF Document Ready for Upload</span>
                    </div>
                  ) : (
                    <div className="space-y-1">
                      <p className="text-xs text-slate-300 font-bold">Click here to Select PDF File or Drag & Drop File</p>
                      <p className="text-[10px] text-slate-500">Supports PDF format (Max 25 MB)</p>
                    </div>
                  )}
                </div>
              </div>
      ...
      {activeTab === 'docs' ? (
        ...
        <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-bold text-white font-heading">Official Gazette Notifications & Document Version Vault</h2>
            <span className="text-xs text-emerald-400 font-bold">
              {documents.length} File(s) Registered & Digitally Signed
            </span>
          </div>
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-800">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-800/80 text-slate-300 font-semibold uppercase tracking-wider">
                <tr>
                  <th className="py-3.5 px-4 border-b border-slate-700">Document Title & File</th>
                  <th className="py-3.5 px-4 border-b border-slate-700">Version</th>
                  <th className="py-3.5 px-4 border-b border-slate-700">Category</th>
                  <th className="py-3.5 px-4 border-b border-slate-700">Verification Status</th>
                  <th className="py-3.5 px-4 border-b border-slate-700">Uploaded Authority</th>
                  <th className="py-3.5 px-4 border-b border-slate-700 text-right">Action</th>
                </tr>
              </thead>
                      ...
                      {isExpanded && hasHistory && docs.slice(1).map((oldDoc, hIdx) => (
                        <tr key={oldDoc.id} className="bg-slate-950/40 hover:bg-slate-800/20 transition">
                          ...
                          <td className="py-2.5 px-4">
                            <span className="text-slate-500 text-[10px]">{oldDoc.doc_type}</span>
                          </td>
                          <td className="py-2.5 px-4">
                            <span className="text-slate-500 text-[10px] italic">Archived</span>
                          </td>
        ...
        /* Document Audit History Log Table */<div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-bold text-white font-heading">Document Access & Modification Audit History</h2>
            <span className="text-xs text-slate-400">Immutable log of document registrations, verifications & gazette revisions</span>
          </div>
