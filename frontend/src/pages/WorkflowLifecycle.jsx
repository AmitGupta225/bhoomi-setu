...
export const WorkflowLifecycle = () => {
  ...
  const getStageActionVocabulary = (stageNumber, status) => {
    if (status === 'Approved') {
      switch (stageNumber) {
        case 1: return "Submit Proposal & Initiate SIA";
        case 2: return "Forward SIA to State Gov";
        case 3: return "Approve R&R Scheme";
        case 4: return "Issue Sec 11 Notification";
        case 5: return "Hear & Resolve Objections";
        case 6: return "Declare Final Acquisition (Sec 19)";
        case 7: return "Pass Valuation Award (Sec 23)";
        default: return `Approve & Advance Stage ${stageNumber}`;
      }
    } else if (status === 'Returned for Re-Scrutiny') {
      switch (stageNumber) {
        case 3: return "Return R&R Scheme to Collector";
        case 6: return "Refer Back to Collector";
        default: return "Return to Proposer for Re-Scrutiny";
      }
    } else if (status === 'Rejected') {
      return `Reject Stage ${stageNumber}`;
    }
  };
    ...
    <div className="p-4 lg:p-8 space-y-8 max-w-full overflow-x-hidden">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-6 rounded-3xl">
        <div className="space-y-1 min-w-0">
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-emerald-400">
            ...
            <span>RFCTLARR Act Statutory Acquisition Engine</span>
          </div>
          <h1 className="text-2xl font-extrabold font-heading text-white truncate">Project Status & Statutory Milestones</h1>
          <p className="text-xs text-slate-400">End-to-end statutory milestones: Proposal Submission, Collector Scrutiny, SIA Hearings, Gazette Declarations, Awards & Payouts.</p>
        </div>
      ...
      <div className="bg-gradient-to-r from-slate-900 via-slate-900 to-slate-950 border border-slate-800 p-6 rounded-3xl space-y-4 shadow-xl">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-800/80 pb-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className="text-slate-400">• {projectDetail.agency}</span>
            </div>
            ...
            <div className="px-5 py-3.5 bg-emerald-950/60 border border-emerald-500/40 rounded-2xl text-right shadow-lg">
              ...
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-500/20 border border-emerald-500/30 rounded-xl text-emerald-300 font-extrabold text-sm shadow-sm">
                <span>Stage {projectDetail.current_stage_id}: {projectDetail.status}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs pt-1">
          ...
          <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-2xl space-y-1.5">
            ...
            <span className="text-[10px] text-slate-500 block">Out of Budget ₹ {projectDetail.estimated_budget_cr} Cr</span>
          </div>

          <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-2xl space-y-1.5">
            <span className="text-slate-400 text-[11px] block">Sponsoring Department:</span>
      ...
      <div className="bg-slate-900/90 border border-slate-800 p-6 rounded-3xl space-y-4 shadow-xl">
        <h2 className="text-lg font-extrabold text-white font-heading flex items-center gap-2">
          <Clock className="w-5 h-5 text-amber-400" />Statutory Deadlines (RFCTLARR)</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { title: 'SIA Notification (Stage 2)', rule: '6 months from Stage 1', target: 2, from: 1, months: 6 },
            { title: 'Sec 11 Gazette (Stage 4)', rule: '12 months from Stage 1', target: 4, from: 1, months: 12 },
            { title: 'Sec 23 Award (Stage 7)', rule: '12 months from Stage 4', target: 7, from: 4, months: 12 },
            { title: 'Sec 24 Compensation Lapse', rule: '24 months from Stage 7', target: 8, from: 7, months: 24 }
          ].map((dl, idx) => {
            ...
            if (!fromStage || !fromStage.approval_date) {
              return (
                <div key={idx} className="p-4 bg-slate-950/80 border border-slate-800 rounded-2xl">
                  ...
                  <div className="text-xs font-semibold text-slate-500 mt-2">Awaiting preceding stage</div>
                </div>
              );
            }
            ...
            return (
              <div key={idx} className="p-4 bg-slate-950/80 border border-slate-800 rounded-2xl">
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  {!isCompleted && <div className="text-[10px] text-slate-400">{daysRemaining} days left</div>}
                </div>
              </div>
            );
          })}
      ...
      <div className="bg-slate-900/90 border border-slate-800 p-6 rounded-3xl space-y-6 shadow-xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-4">
          <h2 className="text-lg font-extrabold text-white font-heading">Statutory Timeline Milestones for:<span className="text-emerald-400">{projectDetail.name}</span>
          </h2>
          <span className="text-xs text-slate-300 font-mono bg-slate-800 px-3 py-1.5 rounded-xl border border-slate-700">Project Code:<strong className="text-emerald-300">{projectDetail.code}</strong></span>
        </div>
      ...
      {activeStage && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-slate-900/90 border border-slate-800 rounded-3xl p-6 space-y-4 min-w-0">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-4">
              <div>
                <span className="text-xs font-mono text-emerald-400 font-bold">Stage {activeStage.stage_number} of 10</span>
            ...
            <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-2xl space-y-2 text-xs">
              <span className="text-[10px] uppercase tracking-wider font-bold text-slate-400 block border-b border-slate-800 pb-1">Active Proposal Technical Parameters</span>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-slate-300 pt-1">
                <div className="min-w-0">
                  ...
                  <strong className="text-white text-xs truncate block">{projectDetail.name}</strong>
                </div>
                <div className="min-w-0">
                  <span className="text-slate-500 text-[10px] block">State & District:</span>
                  <strong className="text-white text-xs block truncate">{projectDetail.state}, {projectDetail.district}</strong>
                </div>
                <div className="min-w-0">
                  <span className="text-slate-500 text-[10px] block">Proposed Land Area:</span>
                  <strong className="text-emerald-400 font-mono text-xs block truncate">{projectDetail.total_land_proposed_ha} Hectares</strong>
                </div>
                <div className="min-w-0">
                  <span className="text-slate-500 text-[10px] block">Estimated Budget:</span>
                  <strong className="text-cyan-400 font-mono text-xs block truncate">₹ {projectDetail.estimated_budget_cr} Cr</strong>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div className="p-3 bg-slate-800/40 border border-slate-700/60 rounded-xl space-y-1">
                <span className="text-slate-400">Assigned Scrutiny Authority:</span>
                <div className="font-semibold text-white truncate">{activeStage.assigned_role}</div>
              </div>

              <div className="p-3 bg-slate-800/40 border border-slate-700/60 rounded-xl space-y-1">
                <span className="text-slate-400">Last Action Date:</span>
                <div className="font-semibold text-emerald-400">{activeStage.approval_date || 'Pending Scrutiny'}</div>
              </div>
            </div>
            ...
            {activeStage.comments && (
              <div className={`p-4 rounded-2xl border text-xs space-y-1 ${activeStage.status === 'Returned for Re-Scrutiny'
                ...
                <span className="font-bold block uppercase tracking-wider text-[10px]">Official Scrutiny Remarks & Objections:</span>
                ...
                {showRectifyForm && (
                  <form onSubmit={handleRectifySubmit} className={`pt-3 border-t space-y-4 text-xs animate-in fade-in ${isCurrentStageReturned ? 'border-amber-500/30' : 'border-slate-700'}`}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="text-slate-300 block mb-1">Estimated Budget (₹ Cr):</label>
                        <input
                          type="number"
                          required
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 space-y-4 min-w-0">
            <div className="flex items-center gap-2 text-xs font-bold text-emerald-400 uppercase tracking-wider">
              ...
              <span>Statutory Scrutiny Console</span>
            </div>

            <div className="p-3 bg-slate-800/40 rounded-xl border border-slate-700 text-xs space-y-1">
              <span className="text-slate-400">Active User Persona:</span>
            ...
            {!isAuthorizedToScrutinizeCurrent ? (
              <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-2xl space-y-2 text-xs text-slate-300">
                <div className="flex items-center gap-2 font-bold text-amber-400">
                  ...
                  <span>Role View-Only Mode</span>
                </div>
                <p className="text-[11px] text-slate-400 leading-relaxed">You are logged in as<strong>{activeRole.badge}</strong>. Official scrutiny and decision for Stage {activeStage.stage_number} is legally assigned to <strong>{activeStage.assigned_role}</strong>.
                  Switch active persona from top right header dropdown to execute decisions.</p>
              </div>
            ) : (
              <>
                <div className="space-y-2">
                  <label className="text-xs text-slate-300 font-medium block">Statutory Scrutiny Remarks & Objections:</label>
                  <textarea
                    rows={3}
                    placeholder="Enter inspection findings, gazette reference numbers, or deficiencies requiring re-scrutiny..."
                    value={comments}
