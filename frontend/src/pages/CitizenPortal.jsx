      ...
      <div className="bg-gradient-to-r from-purple-950/70 via-slate-900 to-slate-900 border border-purple-500/30 p-6 rounded-3xl space-y-3 shadow-2xl">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            ...
            className="px-3 py-1 bg-purple-500/20 hover:bg-purple-500/30 text-purple-300 rounded-lg text-xs font-bold border border-purple-500/30 transition"
          >
        ...
        <div className="space-y-1">
          <h1 className="text-2xl font-extrabold font-heading text-white">
            {t('welcome')}, {user?.name || 'Ramesh Baliram Patil'}
          </h1>
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 space-y-3 shadow-xl">
        <div className="flex items-center justify-between">
          <label className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2 font-heading">
            ...
            <span>{user?.roleKey === 'CITIZEN' ? "Your Registered Land Parcel(s):" : "Select Project Land Parcel to Inspect:"}</span>
          </label>
          {fetchingParcel && (
            <span className="text-[11px] text-purple-400 font-semibold animate-pulse">Fetching parcel ledger...</span>
          )}
        </div>
          ...
          >
            {allParcels.map((p) => (
              <option key={p.id} value={p.ulpin} className="bg-slate-900 text-white">
                {p.ulpin} — Owner: {p.owner_name} ({p.village || p.district}, Area: {p.area_ha} Ha)
              </option>
            ))}
          </select>
        ...
        <div className="p-12 text-center text-slate-400 text-sm font-semibold bg-slate-900/60 rounded-3xl border border-slate-800 animate-pulse">Loading your verified land acquisition record from database...</div>) : activeParcel && (<div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 space-y-6 shadow-2xl animate-in fade-in duration-300">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2 text-xs font-bold text-emerald-400 font-mono">
                ...
                <span>ULPIN Bhu-Aadhaar: {activeParcel.ulpin}</span>
              </div>
              ...
              <p className="text-xs text-slate-400 flex items-center gap-1.5">
                ...
                <span>Survey No. {activeParcel.surveyNumber} (Khata {activeParcel.khataNumber}), {activeParcel.village}, {activeParcel.district}, {activeParcel.state}</span>
              </p>
            </div>

            <div className="text-right space-y-1">
              <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Infrastructure Project</span>
              <span className="px-3 py-1 bg-purple-950/60 text-purple-300 border border-purple-500/30 rounded-xl text-xs font-bold block">
                {activeParcel.projectName}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
            <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-2xl space-y-1">
              <span className="text-slate-400 font-medium">Acquired Plot Area</span>
              <div className="text-lg font-black text-cyan-400">{activeParcel.areaHa} Hectares</div>
              <span className="text-[10px] text-slate-500 block">({(activeParcel.areaHa * 10000).toLocaleString('en-IN')} Sq. Meters)</span>
            </div>

            <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-2xl space-y-1">
              <span className="text-slate-400 font-medium">Land Category & Rate</span>
              <div className="text-sm font-bold text-white truncate">{activeParcel.landType}</div>
              <span className="text-[10px] text-emerald-400 block font-mono">₹ {(activeParcel.marketRateSqm || 4800).toLocaleString('en-IN')} / Sqm</span>
            </div>

            <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-2xl space-y-1">
              <span className="text-slate-400 font-medium">RFCTLARR Statutory Multiplier</span>
              <div className="text-lg font-black text-purple-400">{activeParcel.multiplier || 1.5}x Multiplier</div>
              <span className="text-[10px] text-slate-500 block">Rural/Suburban Multiplier Factor</span>
            </div>

            <div className="p-4 bg-slate-950/80 border border-slate-800 rounded-2xl space-y-1">
              <span className="text-slate-400 font-medium">Acquisition Status</span>
              <div className="text-sm font-bold text-emerald-400 uppercase">{activeParcel.status || 'Possessed'}</div>
              <span className="text-[10px] text-slate-500 block">Statutory Section 19 Approved</span>
            </div>
          </div>
            ...
            <div className="p-5 bg-gradient-to-r from-slate-950 via-slate-950 to-emerald-950/30 border border-slate-800 rounded-2xl space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800/80 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-white font-heading">Area-Scaled Statutory Compensation Computation</h3>
                </div>
                <span className="text-[11px] text-emerald-400 font-medium">Computed dynamically from acquired area</span>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                <div className="p-3 bg-slate-900/60 border border-slate-800 rounded-xl space-y-1">
                  <span className="text-slate-400 font-medium">Calculated Land Value</span>
                  ...
                  <span className="text-[10px] text-slate-500">Area × Market Rate × Multiplier</span>
                </div>

                <div className="p-3 bg-slate-900/60 border border-emerald-500/30 rounded-xl space-y-1">
                  <span className="text-emerald-400 font-bold">100% Solatium Right</span>
                  ...
                  <span className="text-[10px] text-emerald-500">100% Compulsory Bonus</span>
                </div>

                <div className="p-3 bg-emerald-950/50 border border-emerald-500/50 rounded-xl space-y-1">
                  <span className="text-white font-black">Net Statutory Award</span>
                  ...
                  <span className="text-[10px] text-emerald-400">Total Compensation Payable</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 space-y-5 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="text-base font-bold text-white font-heading">Actual Bank Disbursement Status</h2>
                  <p className="text-[11px] text-slate-400">PFMS Direct Benefit Transfer (DBT) Treasury Ledger</p>
                </div>
              </div>
              ...
              <span className={`px-3 py-1 text-xs font-black uppercase rounded-full border ${activeDisbursement.disbursementStatus === 'Disbursed'
                ...
                {activeDisbursement.disbursementStatus === 'Disbursed' ? '✓ Disbursed' : activeDisbursement.disbursementStatus}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
              <div className="p-3.5 bg-slate-950/70 border border-slate-800 rounded-2xl space-y-1">
                <span className="text-slate-400 font-medium">Total Disbursed to Bank</span>
                ...
                <span className="text-[10px] text-slate-500 block">PFMS Treasury Batch Clearance</span>
              </div>

              <div className="p-3.5 bg-slate-950/70 border border-slate-800 rounded-2xl space-y-1">
                <span className="text-slate-400 font-medium">Included Solatium Bonus</span>
                <div className="text-lg font-black text-emerald-400 flex items-center gap-0.5">
                  <IndianRupee className="w-4 h-4 text-emerald-400" />
                  <span>{Math.round(activeDisbursement.solatiumAmount || 0).toLocaleString('en-IN')}</span>
                </div>
                <span className="text-[10px] text-slate-500 block">Compulsory 100% Solatium Transfer</span>
              </div>
            </div>
            ...
            <div className="p-4 bg-slate-950 border border-slate-800/80 rounded-2xl space-y-3 text-xs">
              <div className="flex items-center justify-between text-slate-300 border-b border-slate-800/60 pb-2">
                <span className="text-slate-400 flex items-center gap-1.5">
                  ...
                  <span>PFMS Transaction Ref:</span>
                </span>
                <span className="font-mono font-bold text-cyan-300">{activeDisbursement.pfmsRefNo || 'PFMS-2026-TXN-680002'}</span>
              </div>
              ...
              <div className="flex items-center justify-between text-slate-300">
                <span className="text-slate-400 flex items-center gap-1.5">
                  ...
                  <span>DBT Transfer Credit Date:</span>
                </span>
                <span className="font-semibold text-emerald-400">{activeDisbursement.paymentDate || '2026-08-15'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 space-y-6 shadow-2xl flex flex-col">
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-xs font-bold text-amber-400 uppercase tracking-wider">
                ...
                <span>Online Grievance & Appeal Redressal</span>
              </div>

              <p className="text-xs text-slate-400 leading-relaxed">Have an issue regarding compensation assessment, ownership records, or R&R plot allotment? Submit your petition directly to the District Land Acquisition Authority.</p>
              ...
              {grievanceSubmitted && (
                <div className="p-3 bg-emerald-950/80 border border-emerald-500/40 rounded-xl text-xs font-semibold text-emerald-300 flex items-center gap-2">
                  ...
                  <span>Grievance petition registered successfully!</span>
                </div>
              )}
            </div>
            ...
            <form onSubmit={handleGrievance} className="space-y-3">
              <div>
                <label className="text-xs text-slate-300 mb-1 block font-semibold">Petition / Complaint Description:</label>
                <textarea
                  rows={4}
                  value={grievanceText}
                  onChange={(e) => setGrievanceText(e.target.value)}
                  placeholder="Describe your claim or objection in detail..."
              ...
              >
                ...
                <span>Submit Petition to Collectorate</span>
              </button>
            </form>
            ...
            {myGrievances.length > 0 && (
              <div className="mt-6 pt-6 border-t border-slate-800 space-y-4">
                <h3 className="text-xs font-bold text-white uppercase flex items-center gap-2">
                  ...
                  <span>My Past Petitions ({myGrievances.length})</span>
                </h3>
                    ...
                    <div key={g.id} className="p-3.5 bg-slate-950 border border-slate-800 rounded-xl space-y-2">
                      ...
                      {g.remarks && (
                        <div className="p-2 bg-indigo-950/30 border-l-2 border-indigo-500 text-[11px] text-indigo-300 mt-2">
                          <strong>Collectorate Reply:</strong> {g.remarks}
                        </div>
                      )}
